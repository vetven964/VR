 ...
 *
 * Sequence required before a signal fires (matches the original 1:1):
 ...
 *   4. Displacement      (a strong-bodied candle, >= dispBodyATR * ATR)
 *   + POI (FVG or the displacement candle itself)
...
function runIctEngine(candles, opts = {}) {
  ...
  for (let i = cfg.swingLen; i < n; i++) {
    ...
    if (bullState > 0 && bullStartBar != null && i - bullStartBar > cfg.sequenceBars) { bullState = 0; bullStartBar = null; }
    ...
    if (bullDisp) bullState = 4;

    // --- bear sequence state machine ---
    if (bearSweep) { bearState = 1; bearStartBar = i; }
    if (bearState > 0 && bearStartBar != null && i - bearStartBar > cfg.sequenceBars) { bearState = 0; bearStartBar = null; }
    ...
    const buySignal = bullState >= 4 && bullPOI && bullCandle && buySLValid && buyTargetOK && cooldownOK;
    const sellSignal = bearState >= 4 && bearPOI && bearCandle && sellSLValid && sellTargetOK && cooldownOK;

    if (buySignal) {
      result = { direction: 'BUY', entry: buyEntry, sl: buySL, tp1: buyTP1, barIndex: i, time: cur.time };
      lastSignalBar = i; bullState = 0; bullStartBar = null;
    }
    if (sellSignal) {
      result = { direction: 'SELL', entry: sellEntry, sl: sellSL, tp1: sellTP1, barIndex: i, time: cur.time };
      lastSignalBar = i; bearState = 0; bearStartBar = null;
    }
  }
